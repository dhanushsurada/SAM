...
# Imported before logging.basicConfig() specifically so SAM_DATA_DIR/logs
# already exists (config/settings.py's _ensure_data_dirs() runs at import
# time) before FileHandler tries to open a file inside it below.
...
logging.basicConfig(
    ...
    handlers=[
        # Was the relative path "logs/sam.log" — only ever worked when the
        # process's cwd happened to be the repo root AND that repo root
        # happened to already contain a logs/ directory (it does here, via
        # logs/.gitkeep, which is how this went unnoticed). It also wrote
        # to a completely different file than `sam logs` (sam_cli.py's
        # cmd_logs) reads from, which uses SAM_DATA_DIR — the same
        # ~/.sam_data path config/settings.py's own module docstring
        # documents as where "all persistent data lives", and the same
        # expression Settings.log_path already defaults to. Using the
        # identical expression here means the two can't independently
        # drift apart again.
        logging.FileHandler(str(SAM_DATA_DIR / "logs" / "sam.log")),
