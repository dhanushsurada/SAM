...
window. This is a single static folder served by `interfaces/api/server.py` (the
old `iqoo/server.py` path still works too, as a compatibility shim).
...
Nothing in `interfaces/web/` has been run against a real mobile browser, a real
