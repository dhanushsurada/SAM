"""
SAM Telegram Interface — Bridge

(Moved here in Phase 3A.5 — originated as ecosystem/telegram_bridge.py.
Telegram is a SAM Interface/channel, not device connectivity — the finalized
architecture keeps it out of SAM Connect. ecosystem/telegram_bridge.py now
forwards here; there is only one implementation.)
...
=== IMPORTANT: this is NOT the local-WiFi SAM Connect long-term plan ===
...
zero-third-party pairing (via SAM Connect) is still the long-term plan.

=== Standalone process, on purpose ===
This does NOT run inside main.py's voice/text loop. It's a separate script
you run alongside SAM (`python -m interfaces.telegram.telegram_bridge`),
constructing its own Brain/ReactLoop/FounderMode instances the same way
main.py does. Kept fully separate so nothing about the existing voice/text
pipeline is touched or risked by adding this.
...
3. Run `python -m interfaces.telegram.pair_new_device` to generate a pairing
   QR code.
4. Scan it with your phone's camera — opens Telegram, sends /start <token>
   to the bot automatically.
5. Run this bridge: `python -m interfaces.telegram.telegram_bridge`
6. Message the bot from your phone. Only paired (trusted) chats get a
   response — everyone else gets a polite "not paired" message.

(`python -m ecosystem.telegram_bridge` and `python -m ecosystem.pair_new_device`
still work too — ecosystem/ is now a compatibility shim forwarding here.)
"""
...
logger = logging.getLogger("SAM.Interfaces.Telegram")

NOT_PAIRED_MESSAGE = (
    "This device isn't paired with SAM yet. Ask the owner to run "
    "`python -m interfaces.telegram.pair_new_device` and scan the QR code with this phone."
)
